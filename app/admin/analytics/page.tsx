...
export default async function AnalyticsAdmin(){
  ...
  const {data,error}=await supabase.from('analytics_events').select('session_id,event_name,pathname,target,referrer_host,country_code,device_type,created_at').gte('created_at',since).order('created_at',{ascending:false}).limit(5000);
  ...
  return <main className={styles.page}><div className={styles.wrap}>
    ...
    <p className={styles.guard} style={{marginTop:20}}>Privacy controls are active: rapid duplicate views are ignored, each anonymous session is rate-limited, and events older than 90 days are deleted automatically. Dashboard counts use the newest 5,000 events.</p>
  </div></main>;
}
